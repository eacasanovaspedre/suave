...
let private reqId () = Interlocked.Increment(&nextId)

let private stamp () =
  DateTimeOffset.Now.ToString("HH:mm:ss.fff")

let private log msg = printfn "%s  %s" (stamp ()) msg
...
let hello = GET >=> path "/hello" >=> OK "hello from combinators"
...
let race: WebPart =
  fun ctx ->
    let cache = timeOutMillis 40 |> Alt.afterFun (fun () -> "cache")
    let db = timeOutMillis 400 |> Alt.afterFun (fun () -> "db")
...
let timed: WebPart = timeoutWebPart (TimeSpan.FromSeconds 2.0) slowChild
...
let longRunning: WebPart =
  fun ctx ->
    ...
    let timer =
      ...
      |> Alt.wrapAbortFun (fun () -> log (sprintf "[long #%d] timer nacked" id))
      |> Alt.afterJob (fun () ->
