## Unreleased
* **Breaking:** `WebPart<'a>` is now `'a -> Hopac.Alt<'a option>` instead of `'a -> Async<'a option>`. Sequential routing is `fallback` (`choose` remains an alias). `timeoutWebPart` nacks the child. Connection abort is `HttpContext.abort` / `abortToken`. Socket I/O stays `ValueTask` (`SocketOp`). Write handlers with `webPart { }` or `job { }`. `authenticateBasicAsync` takes `Job<bool>`. `okStream` takes `Job<Stream>`. Validation `JobValidator` replaces `AsyncValidator`. DotLiquid `renderPageFile`/`renderPageString` return `Job<_>`. `startWebServerAsync` listening side is a `Job` (`waitStarted`). `>>=` was dropped from `Suave.Operators`. `<|>` is still sequential try-then. AOT flags dropped. Depends on Hopac 0.5.1. F# `Async` remains only as interop (`ofAsync`/`toAsync`) and BCL `Task` at socket/HTTP-client edges.
